...
layout3 = [[sg.Text('Please enter inputs for experiment 3')],
           ...
           [sg.Text('Fifth parameter for experiment 3 (int): '),sg.Input(key='fifth_param3')]]
...
# Create the Window
def make_win1():
    return sg.Window('Window Title', layout, location = (100,200), finalize=True)
...
while True:
    ...
    if event == 'Start':
        if(layout == 1):
            ...
            # ------------------------------
            # this can be changed to set the automatic x-range of the plot
            x = np.linspace(0, 2 * exp3_params['third_param'])
